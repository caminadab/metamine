...
function optimiseer(exp, issub)
	...
	if false and fnaam(exp) == 'map2' then
		--local num = X('#', arg1(exp))
		local num = arg1(arg1(exp)) or X('#', arg1(exp))
		local nexp = X('llus', num,  arg2(exp))
		return nexp
	end

	-- som -> slus
	if true and fn(exp) == 'Σ' then
		local num = arg1(arg(exp)) or X('#', arg(exp))
		local nexp = X('slus', num)
		return nexp
	end

	-- vouw -> lus
	if true and fnaam(exp) == 'vouw2' then
		local num = X('#', arg1(exp))
		local nexp = X('slus', num,  arg2(exp))
		return nexp
	end
