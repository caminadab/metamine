...
episch1		w = 18 ; breedte\nh = 10 ; hoogte\nframe := 0 ; huidige frame\npos = 0..w × 0..h ; alle blokposities\n\n\n; cirkelpatroon\nf = 2 + 3 · sin(looptijd)\ndx = x - muis.x/10 \ndy = y - muis.y/10\ndst = √(dx²+dy²)\n\n; staat het blok op (x,y) aan?\nisblok = (x,y) → (-dst + looptijd·10) mod 5 < sin(looptijd)+2\nblokken = pos filter isblok\n\n; houdt bij welke frame dit is\nals scherm.ververst dan\n\tframe := frame' + 1\neind\n\nvorm = vierkant((a·10,b·10),9)\ndisco = inkleur(vorm, kleur)\nkleur = rgb(a/10,b/10,sin(looptijd)/2+0.5)\nvierkanten = blokken map ((a,b) → disco)\nuit = teken vierkanten\n\n
episch2		w = 18 ; breedte\nh = 10 ; hoogte\nframe := 0 ; huidige frame\npos = 0..w × 0..h ; alle blokposities\n\n; muisspoor\nsprites := nullen\nnullen = (0..20) map (x → (-1,-1))\n\n; waar de kop te plaatsen (30x/seconde)\nkop = (int(muis.x/10), int(muis.y/10))\n\n; plaats de kop\nals frame mod 2 = 1 dan\n\tsprites := (sprites' vanaf 1) ‖ [kop]\nend\n\n; houdt bij welke frame dit is\nals scherm.ververst dan\n\tframe := frame' + 1\neind\n\nkleur = rgb(1/10,b/10,sin(looptijd)/2+0.5)\n\nvorm = vierkant((a·10,b·10),9)\ndisco = inkleur(vorm, kleur)\nvierkanten = sprites map ((a,b) → disco)\nuit = teken vierkanten
pong	; bal\nx := sw/2\ny := 50\ndx := 0\ndy := random.bereik(-τ/8,τ/8)\n\nals scherm.ververst en (dx > 0 of dx < 0) dan\n\tx := x' + dx'\neind\n\nals scherm.ververst en (dy > 0 of dy < 0) dan\n\ty := y' + dy'\neind\n\nwachttot := looptijd + 1\n\n; variabeles\nvx = 2\nbw = 10\nsw = 10\nsh = 25\nvw = 177.8\n\n; klik\nals muis.klik dan\n\tx := muis.x\n\ty := muis.y\neind\n\n\n; raakt hij de spelers?\nrechts.raakt = (y' > ry' en y' < ry' + sh)\nlinks.raakt = (y' > ly' en y' < ly' + sh)\n\n; rechts stuiter\nals x' ≥ 178 - bw - sw en rechts.raakt dan\n\tdx := - vx\n\tx := 178 - sw - bw\n\tdy := (ry' - y' + sh/2) / 4\neind\n\n; links stuiter\nals x' < 0 + bw + sw en links.raakt dan\n\tdx := vx\n\tx := bw + sw\n\tdy := -(ly' - y' + sh/2) / 4\neind\n\n\n; goal\nals x' < 0 dan\n\tx := 90\n\ty := 50\n\tscore2 := score2' + 1\n\twachttot := looptijd + 1\neind\nals x' > vw dan\n\tx := 90\n\ty := 50\n\twachttot := looptijd + 1\n\tscore1 := score1' + 1\neind\n\n\n; top en onder\nals y' < 0 + bw dan\n\tdy := abs dy'\neind\nals y' > 100 - bw dan\n\tdy := - abs dy'\neind\n\n; score\nscore1 := 0\nscore2 := 0\n\n; spelers\nly := 50 - sh/2\nry := 50 - sh/2\nals toets.omlaag en ry' > 0 dan ry := ry' - 1 eind\nals toets.omhoog en ry' < 100-sh dan ry := ry' + 1 eind\nals toets.s en ly' > 0 dan ly := ly' - 1 eind\nals toets.w en ly' < 100-sh dan ly := ly' + 1 eind\n\n; teken spelers\nlinks = rechthoek((0,ly),(sw,ly+sh))\nrechts = rechthoek((vw-sw,ry),(vw,ry+sh))\nbal = cirkel((x,y),bw/2)\n\nlabel1 = label((90-9,90), alsTekst(score1))\nlabel2 = label((90+3,90), alsTekst(score2))\n\nuit = teken [ links, rechts, bal, label1, label2 ]
; zo tel je vectors met elkaar op\nvplus = (vx,vy),(wx,wy) → vx+wx,vy+wy\n
episch3		v = f(muis.pos, 20)\nuit = teken [v]\n\nals looptijd mod 1 < 1/2 dan\n\tf = vierkant\nanders\n\tf = cirkel\nend
